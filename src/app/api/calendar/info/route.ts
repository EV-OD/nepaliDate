...
export async function GET(request: NextRequest) {
  ...
  const exampleYear = availableYears.length > 0 ? availableYears[0] : 2080;
  ...
  const apiInfo = {
    ...
    documentationUrl: request.nextUrl.origin + "/api-info", // Link to the frontend documentation page
    endpoints: [
      {
        ...
        exampleResponse: "The current JSON response you are viewing (structure may vary based on this current response).",
      },
      {
    ...
    notes: [
      "The API serves data based on pre-scraped JSON files from the 'data' directory.",
      "Dates are provided in Bikram Sambat (BS) and corresponding Gregorian (AD) where applicable.",
      "For the /api/calendar/{YYYY}/{MM} endpoint, 'h: true' indicates a holiday.",
