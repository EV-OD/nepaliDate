...
import type { ApiInfo, ApiEndpoint, DataStructure, DataCoverage, ApiNote } from '@/types'; // Assuming these types exist or will be created
import { getBsYears } from '@/lib/bsCalendarData';
import { NEPALI_MONTHS } from '@/types';

interface ApiInfoGeneratorParams {
  requestOrigin: string;
}

export function generateApiInfoObject({ requestOrigin }: ApiInfoGeneratorParams): any {
  ...
  return {
    ...
    documentationUrl: `${requestOrigin}/api-info`,
    authentication: {
        type: "API Key",
    ...
    endpoints: [
      {
        ...
        parameters: [],
        exampleRequest: `${requestOrigin}/api/calendar/info`,
      ...
      },
      {
        ...
        exampleRequest: `${requestOrigin}/api/calendar/${exampleYear}`,
        exampleResponse: [ 
          {
            bs_year: exampleYear,
            bs_month: 1, 
        ...
        detailedFieldDescriptionsUrl: `${requestOrigin}/api-info#data-structures`, 
        errorResponses: [
      ...
      },
      {
        ...
        exampleRequest: `${requestOrigin}/api/calendar/${exampleYear}/${exampleMonth}`,
        exampleResponse: {
          bs_year: exampleYear,
